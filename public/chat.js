...
async function hasUnreadMessages() {
    const url = baseUrl + "has-unread-messages";
    return fetch(url, {headers: authHeader});
}

async function getMessages(target) {
    const url = baseUrl + `target/${target}`;
    return fetch(url, {headers: authHeader});
}
...
async function markAsSeen(sender) {
    const url = baseUrl + `mark-messages-as-seen/${sender}`;
    return fetch(url, {method: "PUT", headers: authHeader});
}


async function getTargets() {
    const url = baseUrl + "all-messages-targets"
    return fetch(url, {headers: authHeader});
}
    ...
    getTargets()
        ...
        .then(r => {
            if(r.length > 0) {
                ...
                activeUsers.innerHTML = "";
                r.forEach(t => {
                    activeUsers.appendChild(createUserElement(t.id, t.nickname));
                    addEventToUserElement(t.id);
                })
